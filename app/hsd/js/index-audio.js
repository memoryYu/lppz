...
//					me.musicArr.push(random(me.nowCountry*5+1,(me.nowCountry+1)*5+1));
		...
		musicPage.act = {
			...
			preloadMusic : function(musicid,readyCb){
				...
				if(this.myVid){
					me.myVid.pause();
					try{
						me.myVid.currentTime = 0;
					}catch(e){
						me.myVid.src = musics[me.nowMusicId];
						me.myVid.load();
						me.$musicLoad.show();
					}
				}
				...
				var src = musics[musicid];
//				this.audioPlayer = document.getElementById("audioPlayer");
				console.log(musicsSM2);
				if(musicsSM2[musicid]){//这首歌非第一次进来
					console.log(2);
					console.log(musicid);
					me.myVid = musicsSM2[musicid];
					me.myVid.play();
					++type.times;
					...
					console.log(1);
					me.myVid = new Audio(src);
					me.myVid.loop = true;
					me.myVid.preload = true;
					me.myVid.load();
//					me.myVid.addEventListener("canplaythrough", function(event){
//						me.myVid.removeEventListener('canplaythrough');
//						console.log(event.path[0].src);
//						console.log(temp + "歌曲加载完成！");
//						me.$musicLoad.hide();
//						readyCb && readyCb(temp);
//						musicsSM2[musicid] = me.myVid;
//					});
					var inter = self.setInterval(function(){
						if(me.myVid.readyState == 4){
							console.log(me.myVid.src);
							console.log(temp + "歌曲加载完成！");
							me.$musicLoad.hide();
							readyCb && readyCb(temp);
							musicsSM2[musicid] = me.myVid;
			...
			checkTouch : function(){
				...
				this.$changeMusic.on(config.touch,function(){
					...
					me.preloadMusic(me.randMusic(),function(musicid){
						me.playMusic(musicid);
					});
...
//					alert("data: " + JSON.stringify(data));
		...
		//分享
        function wxshareType(chance, isshare ,isgoto) {
        	var temp = null;
            if (chance <= 0) { //没有机会
                if (isshare <= 0) { // 没有分享
                    temp = 1; 
            ...
            share(config.shareInfo,function(){
                 shareSuccCb(temp,isgoto); //+1的操作
             }); 
        }
        function shareSuccCb(type,isgoto){
            if(type){
                post('hsd/record', {
                    type: 0
                },function(data){
                    ...
                    switch(err){
                        case 0:
							...
							successPop.act.out();
							$chance = 1;
//							gotoUrl('http://mp.weixin.qq.com/s?__biz=MzA3MTg0NDkyNQ==&mid=210706335&idx=1&sn=2b1f6b328806291fbbd209547b79916f&scene=23&srcid=0915SVYpoLWr0D68hL44pmfo#rd');
                            break;
                        default:
                            defaultError(data);
                    }

(function(){
	require([config.configUrl],function(){
		var reqArr = ['auth','MHJ','loading','ngapi','imgpreload','jquery','wxshare','../hsd/js/soundmanager.min.js?v=201507091117','swiper','metaFix'];
		require(reqArr,requireCb);
	});
	
	function requireCb(oAuth,MHJ,M,ngapi,imgpreload,$,wxshare,soundManager,swiper){
		var indexPage = {}; //首页对象
		var yaoPage = {}; //摇色子页面对象
...
//				yaoPage.act.preloadMusic();
		...
		yaoPage.act = {
			...
			init : function(){
				...
				this.checkTouch();
				this.$dom.show();		
//				this.preloadMusic(this.nowCountry);
				console.log(yaoPage);
			},
			checkTouch : function(){
				var me = this;
...
//				});
			...
			ranYao : function(){
				...
				$('#yao,.shaizi').on(config.touch,function(){ //首先应该根据国家获取之前随机数组中的数据，然后再次生成数据
					...
					console.log(me.musicArr[me.nowCountry]);
					this.touchTimes++;
					var dice = $('#dice');
					...
					onready : function(status){
						if(status.success){
								soundManager.createSound({
									id : 'music_'+me.musicArr[country],
									autoLoad : true,
									autoPlay: false,
									useHTML5Audio:true,
									volume: 100,
									url: musics[me.musicArr[country]], //mp3文件的位置
									onload : function(istrue){
										if(istrue){
											console.log(this.id + '歌曲加载成功');
											me.$musicLoad.hide();
										}else{
											alert(this.id + '歌曲加载失败');
										}
//										console.log(this.id + (istrue?'歌曲加载完毕':'歌曲加载失败'));
...
//									},
								...
								console.log("musicArr: " + me.musicArr);
...
//				alert(musicid);
				...
				this.preloadMusic(musicid,null);
				this.checkTouch();
...
//				this.$musicLoad.show();
				...
				soundManager.setup({
					onready : function(){
						...
						console.log("musicsSM2: " + musicsSM2);
//						console.log(musicsSM2[musicid]);
//						alert(musicsSM2[musicid]);
...
//								},
								...
								onload : function(istrue){
//									var istrue = this.bytesLoaded/this.bytesTotal;
									tag++;
									if(tag<=1){
										if( istrue == 1){
											console.log(this.id + '歌曲加载成功！');
...
//										istrue ? console.log(this.id+'歌曲加载完毕') : alert(this.id+'歌曲加载失败');
//										me.$musicLoad.hide();
...
//				soundManager.stopAll();
			...
			checkTouch : function(){
				...
				//换一首歌
				this.$changeMusic.on(config.click,function(){
					soundManager.stopAll();
					me.$musicLoad.show();
					$('#playMusic').addClass('active');
...
//					alert("x: " +x+" "+"y: "+y+" " +bangArr[x][y]);
		...
		failPop.act = {
			...
			checkTouch: function() {
				var me = this;
				this.$share.on(config.click, function() {
					$('#popshare').show();
					$('#popshare').on(config.click, function() {
						$('#popshare').hide();
					});
				});
				this.$back.on(config.click, function() {
					me.out();
					yaoPage.act.init();
				});
				this.$playAgain.on(config.click,function(){
					me.out();
					gamePage.act.init();
				});
			},
...
//				this.checkTouch();
			...
			checkTouch: function() {
				...
				this.$checkRank.on(config.click, function() {
					me.out();
					rankPage.act.init();
				});
				this.$playAgain.on(config.click, function() {
